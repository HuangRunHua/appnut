...
impl UiStrings {
    fn new() -> Self {
        ...
        m.insert(
            "ui/login/title",
            ["Welcome back", "欢迎回来", "おかえりなさい", "Bienvenido"],
        );
        m.insert(
            "ui/login/subtitle",
            [
        ...
        m.insert(
            "ui/login/username",
            ["Username", "用户名", "ユーザー名", "Usuario"],
        );
        m.insert(
            "ui/login/password",
            ["Password", "密码", "パスワード", "Contraseña"],
        );
        m.insert(
            "ui/login/button",
            ["Sign In", "登录", "サインイン", "Iniciar sesión"],
        );
        m.insert(
            "ui/login/hint",
            [
                "Try: alice, bob, or carol",
                "试试: alice, bob, carol",
                "alice, bob, carol を試してください",
                "Prueba: alice, bob o carol",
            ],
        );
        ...
        m.insert(
            "ui/compose/cancel",
            ["Cancel", "取消", "キャンセル", "Cancelar"],
        );
        m.insert(
            "ui/compose/reply_placeholder",
            [
                "Write a reply...",
                "写回复...",
                "返信を書く...",
        ...
        m.insert(
            ...
            ["Followers", "粉丝", "フォロワー", "Seguidores"],
        );
        m.insert(
            "ui/profile/following",
            ["Following", "关注", "フォロー中", "Siguiendo"],
        );
        m.insert(
            "ui/profile/tweets",
            ["Tweets", "推文", "ツイート", "Tweets"],
        );
        m.insert(
            "ui/profile/follow",
            ["Follow", "关注", "フォロー", "Seguir"],
        );
        m.insert(
            "ui/profile/unfollow",
            ["Unfollow", "取消关注", "フォロー解除", "Dejar de seguir"],
        );
        m.insert(
            "ui/profile/no_tweets",
            [
                "No tweets yet",
                "还没有推文",
                "ツイートはまだありません",
        ...
        m.insert(
            "ui/me/sign_out",
            ["Sign Out", "退出登录", "サインアウト", "Cerrar sesión"],
        );
        m.insert(
            "ui/me/admin_dashboard",
            [
        ...
        m.insert(
            "ui/me/developer",
            ["Developer", "开发者", "開発者", "Desarrollador"],
        );

        // Edit profile
        m.insert(
            "ui/edit/display_name",
            ["Display Name", "显示名称", "表示名", "Nombre"],
        );
        m.insert("ui/edit/bio", ["Bio", "简介", "自己紹介", "Biografía"]);
        m.insert(
            "ui/edit/save",
            ["Save Changes", "保存修改", "変更を保存", "Guardar cambios"],
        );
        m.insert(
            "ui/edit/saved",
            ["Saved!", "已保存！", "保存しました！", "¡Guardado!"],
        );
        ...
        m.insert(
            ...
            [
                "Search users or tweets...",
                "搜索用户或推文...",
                "ユーザーやツイートを検索...",
                "Buscar usuarios o tweets...",
            ],
        );
        m.insert("ui/search/users", ["Users", "用户", "ユーザー", "Usuarios"]);
        m.insert(
            "ui/search/tweets_section",
            ["Tweets", "推文", "ツイート", "Tweets"],
        );
        m.insert(
            "ui/search/no_results",
            ["No results", "没有结果", "結果なし", "Sin resultados"],
        );
        ...
        m.insert(
            "ui/inbox/title",
            ["Inbox", "站内信", "受信箱", "Bandeja de entrada"],
        );
        m.insert(
            "ui/inbox/empty",
            [
    ...
    fn new() -> Self {
        ...
        m.insert(
            ...
            [
                ...
                "推文不能为空",
                "ツイートは空にできません",
                "El tweet no puede estar vacío",
            ],
        );
        m.insert(
            "error/tweet/too_long",
            [
                "Tweet exceeds 280 characters",
                "推文超过280个字符",
                "ツイートは280文字を超えています",
                "El tweet supera los 280 caracteres",
            ],
        );
        m.insert(
            "error/profile/name_empty",
            [
...
impl I18nHandler for ErrorStrings {
    fn translate(&self, path: &str, query: &QueryParams, locale: &str) -> String {
        ...
        if let Some(username) = query.get("username") {
            return base
                .replace("not found", &format!("'{}' not found", username))
